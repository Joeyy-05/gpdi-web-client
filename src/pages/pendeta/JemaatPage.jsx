...
import { deleteJemaat, createJemaat, updateJemaat } from '../../services/jemaatService';    
...
const JemaatPage = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setErrorMsg(typeof error === 'string' ? error : "Gagal mengambil data jemaat.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleRoleChangeInline = async (id, newRole, userName) => {
        if (window.confirm(`Apakah Anda yakin ingin mengubah peran ${userName || 'pengguna ini'} menjadi ${newRole.replace('_', ' ')}?`)) {
            try {
    ...
    const handleOpenAddModal = () => {
        setEditId(null);
        setFormData({ name: '', email: '', password: '', role: 'jemaat', id_rayon: '' });
        setErrorMsg('');
        setIsModalOpen(true);
    };
    ...
    const handleSubmitForm = async (e) => {
        ...
        try {
            ...
            if ((formData.role === 'jemaat' || formData.role === 'ketua_rayon') && !formData.id_rayon) {
        ...
        <div className="font-sans space-y-6 relative">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Kelola data anggota, ploting rayon, dan hak akses (Role).</p>
                </div>
                <button 
                    ...
                    <table className="w-full whitespace-nowrap text-left text-sm text-slate-600">
                        <thead className="bg-slate-50 text-slate-700 border-b border-slate-200">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-4 font-semibold">Rayon</th>
                                <th scope="col" className="px-6 py-4 font-semibold">Peran Akses</th>
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {isLoading ? (
                                ...
                                <tr><td colSpan="5" className="px-6 py-8 text-center">Tidak ada data jemaat.</td></tr>
                            ) : (
                                    ...
                                    <tr key={item.id} className="hover:bg-slate-50 transition">
                                        ...
                                        <td className="px-6 py-4">
                                            <select 
                                                ...
                                                onChange={(e) => handleRoleChangeInline(item.id, e.target.value, item.name)}
                                                className={`px-3 py-1.5 rounded-md text-xs font-semibold border capitalize cursor-pointer transition focus:outline-none focus:ring-2
                                                    ${item.role === 'pendeta' ? 'bg-purple-50 text-purple-700 border-purple-200 focus:ring-purple-500' 
                                                    : item.role === 'ketua_rayon' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 focus:ring-emerald-500' 
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <div className="flex justify-center space-x-2">
                                                <button onClick={() => handleOpenEditModal(item)} className="text-amber-500 hover:text-amber-600 bg-amber-50 hover:bg-amber-100 p-2 rounded-lg transition" title="Edit Profil">
                        ...
                        <form onSubmit={handleSubmitForm} className="p-6 space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-1">
                                    Kata Sandi {editId && <span className="text-slate-400 font-normal">(Kosongkan jika tidak diubah)</span>}
                                </label>
                                <input 
                                    type="password" 
                                    required={!editId} 
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-1">Plotting Rayon</label>
                                    <select 
                                        value={formData.id_rayon} 
                                        onChange={(e) => setFormData({...formData, id_rayon: e.target.value})}
