...
export default function PublicNavbar() {
  ...
  return (
    <header
      className="w-full bg-[#0D1282] text-white sticky top-0 z-50 shadow-md"
      ...
      <div className="mx-auto flex h-[78px] w-full max-w-[1440px] items-center justify-between px-6 lg:px-10">
        <Link to="/" className="flex flex-shrink-0 items-center gap-3">
          ...
          <img
            src={logo}
            alt="Logo GPdI"
            className="h-[40px] w-[40px] object-contain rounded-full bg-white p-0.5"
          />
          <span className="hidden text-[18px] font-bold sm:block">
            GPdI Sibulele
          </span>
        </Link>
        ...
        <nav className="hidden flex-1 items-center justify-center gap-x-4 lg:flex xl:gap-x-6">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `whitespace-nowrap text-[14px] transition-all py-2 border-b-2 ${isActive ? "font-bold border-white" : "font-medium border-transparent hover:border-white/50"}`
              }
            >
              {item.label}
            </NavLink>
        ...
        <div className="flex items-center gap-4">
          <Link
            to="/login"
            className="rounded-full bg-[#D71313] px-6 py-2.5 text-[14px] font-bold transition hover:bg-[#b51010] shadow-sm"
          >
            LOGIN
          </Link>
        </div>
